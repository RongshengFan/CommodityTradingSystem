...
public:
	...
	// 3.2 定义ID查询接口处理
	API_HANDLER_ENDPOINT_AUTH(API_M_GET, "/sample/query-by-id", querySampleById, QUERY(Int64, id), execQueryById(id));
	...
	API_HANDLER_ENDPOINT_AUTH(API_M_GET, "/sample/query-one", queryOne, QUERY(UInt64, id), c(id, authObject->getPayload()));
...
private:
	...
	// 3.3 演示ID查询数据
	SampleJsonVO::Wrapper execQueryById(const Int64& id);
	// 3.3 演示新增数据
	Uint64JsonVO::Wrapper execAddSample(const SampleAddDTO::Wrapper& dto);
	// 3.3 演示修改数据
